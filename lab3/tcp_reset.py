...
class Hacker:
    ...
    def send_rst_flag(self, ip, port):
        print(f'{self}: sent rst to server {ip}:{port}')
    ...
    def __str__(self):
        return f'\nHacker ip={self._hacker_client.ip} ports={self._hacker_client.ports[0]} ' \
               f'nvictim ip={self._client.ip} port={self._client.ports[0]})'
